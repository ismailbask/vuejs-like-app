    ...
    <div class="p-3">
      ...
      <div class="flex items-center justify-center mt-2 gap-x-1">
        ...
        <button
          ...
          :class="{ 'bookmark-item-active': allReadyBookmarked }"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            class="fill-current group-hover:text-white"
...
export default {
  ...
  methods: {
    likeItem() {
      this.$appAxios({
        url: this.allReadyLiked
          ? `/user_likes/${this.likedItem.id}`
          : "/user_likes",
        method: this.allReadyLiked ? "DELETE" : "POST",
        data: { userId: this._getCurrentUser.id, bookmarkId: this.item.id },
    ...
    _likeItem() {
      ...
      // console.log(this.allReadyLiked);
      if (!this.allReadyLiked) {
        //like etmemişse
        likes = [...likes, this.item.id];
        // console.log("likes", this._userLikes);
      } else {
        //like etmişse ve tekrardan butona basarsa filter ile item.id değerini çıkarıyoruz.
        likes = likes.filter((l) => l !== this.item.id);
        // console.log("likes", likes);
      }
      //1-)likes arrayına storedan gelen _userLike değeri ve şuanki beğenile item.id eklenir.
      this.$appAxios
        //3-)böylelikle likes arrayı endpoint ile doğru bir şekilde güncellenmiş olur. bu array normal de like işlemi yapılınca item.id'yi üzerine eklemek yerine güncelleme yapıyordu.
        .patch(`/users/${this._getCurrentUser.id}`, { likes })
        .then((like_response) => {
          console.log(like_response);
          //2-)işlem başarılı olursa anlık olarak storedaki user.likes arrayı değiştirilir.
  ...
  computed: {
    //   categoryName() {
    //     return this.item?.categoryId?.name || "-";
    //   },
    //   userName() {
    ...
    //     //_userLikes'ın içinde this.item.id değeri varsa true döner. her eklenmiş olan işlem için döner.
    //   },
    //   allReadyBookmarked() {
